// translate.controller.ts — Handles /translate and /translate/batch request validation and response

import type { Request, Response } from 'express';

import { ALLOWED_MODELS } from '@/config/models';
import { logger } from '@/lib/logger';
import { translateBatchSegments, translateSegments } from '@/services/translate.service';
import type { BatchTranslateRequest, TranslateRequest } from '@/types';

export const translateController = async (req: Request, res: Response): Promise<void> => {
  const { blockType, segments, contextBlocks, targetLanguage, provider, model, userContext } =
    req.body as Partial<TranslateRequest>;

  if (!blockType || !segments?.length || !targetLanguage || !provider || !model) {
    res
      .status(400)
      .json({ error: 'blockType, segments, targetLanguage, provider, and model are required' });
    return;
  }

  const knownProviders = Object.keys(ALLOWED_MODELS);
  if (!knownProviders.includes(provider)) {
    res
      .status(400)
      .json({ error: `Unknown provider "${provider}". Allowed: ${knownProviders.join(', ')}` });
    return;
  }

  const allowedModels = ALLOWED_MODELS[provider];
  if (!allowedModels.includes(model)) {
    res.status(400).json({
      error: `Model "${model}" is not allowed for provider "${provider}". Allowed: ${allowedModels.join(', ')}`,
    });
    return;
  }

  logger.request(req.method, req.path, req.body);

  try {
    const translated = await translateSegments({
      segments,
      contextBlocks,
      targetLanguage,
      provider,
      model,
      userContext,
    });
    logger.success(req.method, req.path, 200, { segments: translated });
    res.json({ segments: translated });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Translation failed';
    logger.error(req.method, req.path, 500, message);
    res.status(500).json({ error: message });
  }
};

export const translateBatchController = async (req: Request, res: Response): Promise<void> => {
  const { blocks, targetLanguage, provider, model, userContext } =
    req.body as Partial<BatchTranslateRequest>;

  if (!blocks?.length || !targetLanguage || !provider || !model) {
    res
      .status(400)
      .json({ error: 'blocks, targetLanguage, provider, and model are required' });
    return;
  }

  const hasInvalidBlock = blocks.some((b) => !b.blockType || !b.segments?.length);
  if (hasInvalidBlock) {
    res.status(400).json({ error: 'Each block must have blockType and segments' });
    return;
  }

  const knownProviders = Object.keys(ALLOWED_MODELS);
  if (!knownProviders.includes(provider)) {
    res
      .status(400)
      .json({ error: `Unknown provider "${provider}". Allowed: ${knownProviders.join(', ')}` });
    return;
  }

  const allowedModels = ALLOWED_MODELS[provider];
  if (!allowedModels.includes(model)) {
    res.status(400).json({
      error: `Model "${model}" is not allowed for provider "${provider}". Allowed: ${allowedModels.join(', ')}`,
    });
    return;
  }

  logger.request(req.method, req.path, req.body);

  try {
    const result = await translateBatchSegments({
      blocks,
      targetLanguage,
      provider,
      model,
      userContext,
    });
    logger.success(req.method, req.path, 200, result);
    res.json({ blocks: result });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Batch translation failed';
    logger.error(req.method, req.path, 500, message);
    res.status(500).json({ error: message });
  }
};
