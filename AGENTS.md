...
Codex can delete files directly using the shell when a file is no longer needed.
...
Tasks are tracked in `working-tasks/`. Numbered `.md` files (e.g. `01-feature.md`) are written by the user. Codex's working file is prefixed with `_` so it sorts to the top.

**Codex's working file:** `working-tasks/_progress.md`
